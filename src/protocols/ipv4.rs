...
impl Ipv4Header{
    pub fn new(packet: &Ipv4Packet) -> Ipv4Header{
        ...
        data.extend((0..16).rev().map(|i| ((packet.get_total_length() >> i) & 1) as i8));
        data.extend((0..16).rev().map(|i| ((packet.get_identification() >> i) & 1) as i8));
        data.extend((0..3).rev().map(|i| ((packet.get_flags() >> i) & 1) as i8));
        data.extend((0..13).rev().map(|i| ((packet.get_fragment_offset() >> i) & 1) as i8));
        data.extend((0..8).rev().map(|i| ((packet.get_ttl() >> i) & 1) as i8));
        data.extend((0..8).rev().map(|i| ((packet.get_next_level_protocol().0 >> i) & 1) as i8));
    ...
    pub fn get_headers()-> Vec<String>{
        ...
        fields.iter().flat_map(|(name, bits)| {
            (0..*bits).map(move |i| format!("{}_{}", name, i))
        }).collect()
    }
}
